...
class gbXML():
    ...
    def get_shades(self):
        ...
        for device in shadowsparts:
            ...
            if check == "Shade":
                ...
                typec = str(typec)
                #print "looking for", category, typec
                default_idu = ""
                ...
                for c in construction_names:  # Use category type to find match in existing construction types
                    ...
                    #Added Here:
                    default_idu = c.get("id")  # Added this to test defaults
                    ...
                    if name.find(typec) != -1:
                        ...
                        #print "set here..........................", new_shade.obj_constr, new_shade.obj_type
                        break  #??
                    ...
                    #        new_shade.obj_constr = c.get("id")
                    #        new_shade.obj_type = category
                #print "correct? ", new_shade.obj_type
                if new_shade.obj_type == "Shade":
                    ...
                    new_shade.obj_constr = default_idu # "cons-1" is not a guaranteed label (maybe Revit versioning) so will use a default that is sure be in the file
    ...
    def get_spaces(self):
        ...
        for space in spaces:
            ...
            #print "adding surfaces?", new_space.surfaces
    ...
    def getRectangularGeometry(self,element,parent_node):
        ...
        if not parent_node.xpath("./gb:RectangularGeometry/gb:Azimuth",namespaces = self.namespaces):
            #print "Case where no azimuth in version 2016"
            element.azimuth = float(0.00)  # In 2014, OPENINGS are the problem but all seem to be 0.00 relative to the surface they are nested in, so using 0.00 default at this time
        else:
            #print "Case where no tilt in version 2016"
            element.azimuth = float(parent_node.xpath("./gb:RectangularGeometry/gb:Azimuth",namespaces = self.namespaces)[0].text)
        if not parent_node.xpath("./gb:RectangularGeometry/gb:Tilt",namespaces = self.namespaces):
            element.tilt = float(0.00)  # In 2014, OPENINGS are the problem but all seem to be 0.00 relative to the surface they are nested in, so using 0.00 default at this time
        else:
        ...
        for bounding in space_boundaries:
            ...
            for surface in surfaces:
                ...
                if check == new_check.check_obj_id:
                    ...
                    #print "SURFS..............", new_surface.obj_id
